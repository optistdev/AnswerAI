...
const Sidebar = () => {
  ...
  const selectedItemFromPath = sideBarItems.find((item) =>
    location.pathname.startsWith(item.path)
  )?.id;
  const [selectedItem, setSelectedItem] = useState(() => {
    return (
      localStorage.getItem("sidebarSelected") || selectedItemFromPath || "home"
    );
  });
        ...
        >
          ...
          `}
            >
              ...
              <button className="px-4 py-2 hover:bg-[#3a3a3a] text-left cursor-pointer">
                My Account
              </button>
              <button
                className="px-4 py-2 hover:bg-[#3a3a3a] text-left cursor-pointer"
